...
// command to install the secp256o1 library
...
{ time_t Now; time(&Now);
  ...
  printf("SecKey[32] = "); PrintBytes(SecKey, 32); printf("\n");         // print the secret key
  ...
  printf("MsgSig[64] = "); PrintBytes(CompSig, 64); printf("\n");    // (hex-printable)
  ...
  printf("MsgRecSig[64] = "); PrintBytes(CompRecSig, 64); printf(" RecID:%02X\n", RecID);          // (hex-printable)
  ...
  printf("RecPubKey[%lu] = ", PubKeyLen);
  PrintBytes(ComprPubKey, PubKeyLen); printf("\n"); // (hex-printable)
